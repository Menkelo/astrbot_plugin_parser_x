...
| 抖音 | 原生 | `core/parsers/douyin/` | 视频、图文、图集、Cookie 与 yt-dlp 兜底；视频正文卡和视频按完成顺序发送，图文单图直接引用原图，多图、无封面正文卡与评论长图合并转发；作品元数据表情可进入长卡，`comment_feed.py` 适配 Cookie 评论接口、A-Bogus、表情、图片、贴纸、作者标识与楼中楼 |
| 快手 | 原生 | `core/parsers/kuaishou.py` | 视频、图片、图文均使用统一长卡，视频文件独立发送 |
| 微博 | 原生 | `core/parsers/weibo.py`、`core/parsers/weibo_comment.py` | 完整正文使用统一正文卡，正文 HTML 表情按图片渲染；无视频图文单图直接引用原图，多图、无封面正文卡与评论长图合并转发，带视频微博的正文卡与视频按完成顺序发送；公开 `m.weibo.cn/comments/hotflow` 评论支持认证/VIP、微博表情、原博与楼中楼 |
| 小红书 | 原生 | `core/parsers/xiaohongshu.py` | 视频、图片、图文均使用统一正文卡；图文单图直接引用原图，多图与无封面正文卡合并转发，空正文不回填分享链接，视频文件独立发送，时间戳兼容秒和毫秒，清理源数据的 `[话题]` 内部标记 |
...
| 米游社 | 原生 | `core/parsers/miyoushe.py`、`core/parsers/miyoushe_comment.py` | 文章正文、封面和正文配图合成一张图文一体长卡，成功后不再发送原生图片批次，视频仍独立；官方表情与评论自定义表情按图片渲染；公开 `getPostReplies` 评论支持认证、等级、配图与楼中楼并使用独立评论图合并转发 |
| 小黑盒 | 原生 | `core/parsers/xiaoheihe.py`、`core/parsers/xiaoheihe_comment.py` | 帖子正文、封面和富文本配图按顺序合成一张图文一体长卡，成功后不再重复发送概要或富文本转发，官方表情按图片渲染并过滤客户端本地缓存路径，视频仍独立；无 Cookie 也先尝试签名接口，失败后回退公开接口/分享页，帖子评论使用独立评论图合并转发 |
...
- 五个平台共享评论数量与超时配置；评论任务与主内容同时准备，但不会进入正文卡。抖音和微博
  的纯图文作品会把正文卡与评论图并入原图转发，其余作品仍在主内容完成后发送独立评论转发；
...
- 合并转发使用 AstrBot `Nodes`/`Node` 组件，由 aiocqhttp 适配器转换为
  ...
  一个节点。独立评论转发使用一个标题节点加若干图片节点，图文分离作品则直接把评论图并入
  正文卡所在的原图转发。
