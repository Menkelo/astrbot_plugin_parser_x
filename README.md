...
- Bilibili：视频、分 P、动态/opus、直播原生消息链、可选评论区卡片；所有 HTML 卡片统一使用
...
- 快手：视频、图片和图文作品。
- 微博：正文使用统一概览卡，图片和视频按原生批次分开发送；单图引用原消息。公开热门评论卡片支持认证/VIP、
  原博标识、微博表情、作者点赞和首条楼中楼。
- 小红书：视频、图片和图文笔记；图文正文使用统一概览卡。
- 米游社：文章统一概览卡、图片与视频原生消息批次，以及公开热门评论区。
- 小黑盒：帖子与游戏统一概览卡、富文本图文顺序和帖子评论区；公开内容会先尝试签名接口，
...
动态与评论卡片使用同一“极简信息流”骨架，并复用 AstrBot 官方 Canvas/T2I；平台差异仅保留
品牌色与语义字段，直播使用原生图片与文字消息链。插件不安装、
