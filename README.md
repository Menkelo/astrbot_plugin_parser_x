...
- 微博：完整正文使用统一长卡；长卡单独引用原消息，原图和视频按各自批次随后发送。公开热门
  评论支持认证/VIP、原博标识、作者点赞和首条楼中楼；正文及评论中的微博表情均按图片渲染。
- 小红书：视频、图片和图文笔记均使用统一长卡，视频文件独立发送，发布时间兼容秒和毫秒
  时间戳；平台内部的 `[话题]` 标记会清理为标准 `#话题#`。
...
固定平台代表色磨砂顶栏、主视觉、真实互动数据、作者、正文、作品信息、热门评论和
`Menkelo/astrbot_plugin_parser_x` 页脚；顶栏不再受作品图片颜色影响，并叠加半透明高光、
细颗粒和模糊层，不再显示 B/米/微等字母或单字平台标记。直播继续使用原生消息链；视频作品会先发送统一长卡，
再独立发送原生视频文件。
