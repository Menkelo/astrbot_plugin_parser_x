...
- 小红书：视频、图片和图文笔记；视频直接发送，图文单图引用原消息，多图使用合并转发；
  ...
  为标准 `#话题#`。
- 米游社：保留文章正文、封面、正文配图和视频的原生图文批次，不生成正文卡；含视频时可
  并发发送独立评论图。官方表情和用户自定义评论表情会按图片渲染。
- 小黑盒：保留帖子概要、富文本正文、配图、平台表情和视频的原生顺序，不生成或重复发送
  正文卡；含视频时可并发发送独立评论图。客户端本地缓存路径会被过滤；
...
运行环境还应提供 `ffmpeg`，用于音视频合并、格式转换和 H.264 发送兜底。插件不再生成正文
长卡：图文分离平台只发送作品原图或视频，小黑盒与米游社保留自身的图文兼并消息结构；
独立评论区图片仍使用 AstrBot 官方 `html_render`（Canvas/T2I）渲染。
...
与米游社保留原生图文兼并批次。评论渲染使用 AstrBot 官方 `html_render`。快手和小红书评论
接口依赖频繁变化的私有签名与
