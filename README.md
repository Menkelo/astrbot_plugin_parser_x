...
- 微博：完整正文使用统一长卡；单图只在长卡内显示，多图和视频按各自批次随后发送。公开热门
...
- 米游社：文章使用统一长卡；单图只在卡片内显示，多图和视频按原生批次随后发送，公开热门
...
视频兼容层会发送统一长卡和独立视频文件；网易云音乐会先发送统一长卡，再独立发送音频文件。
...
固定平台代表色纯色顶栏、主视觉、真实互动数据、作者、正文、作品信息、热门评论和
`Menkelo/astrbot_plugin_parser_x` 页脚；顶栏不再受作品图片颜色影响，也不再使用磨砂、颗粒、
模糊或 B/米/微等字母和单字平台标记。直播继续使用原生消息链；视频作品会先发送统一长卡，
再独立发送原生视频文件。
...
评论长图。统一长卡始终作为引用原消息的独立消息；单图作品由卡片主视觉完整承载且不再重复
发送，多图、富文本转发和视频继续使用后续独立批次，合并转发中的每段文字和每张图片各占一个节点。
