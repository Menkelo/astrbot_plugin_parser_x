...
- 微博：完整正文使用统一长卡，正文卡和图片合并为同一个消息组，单图消息引用原消息，视频
  独立发送；公开热门评论支持认证/VIP、原博标识、微博表情、作者点赞和首条楼中楼，并优先
  嵌入正文卡。
- 小红书：视频、图片和图文笔记；图文使用统一长卡，发布时间兼容秒和毫秒时间戳。
- 米游社：文章使用统一长卡，正文卡与图片合并为同一个消息组，视频独立发送，公开热门评论
  优先嵌入长卡。
- 小黑盒：帖子与游戏使用统一长卡，正文卡与富文本按原图文顺序合并发送，视频独立发送，
  帖子评论优先嵌入长卡；
