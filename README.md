...
- Bilibili：视频、分 P 及所有动态使用统一纵向正文卡；图文动态单图直接引用原图，多图将
  原图逐张放入合并转发，无封面正文卡作为附属节点随多图转发或另行放入转发；转发动态会
  补回原动态正文与图片，直播保持原生消息链。所有
...
- 抖音：视频、图文和图集；短链跳转；Cookie 与 yt-dlp 兜底。视频正文卡与视频文件独立
  发送；图文单图直接引用原图，多图、无封面正文卡和评论长图合并转发。配置 `douyin_ck`
  后支持作品/评论表情、作者标识、评论图片、贴纸和首条楼中楼。
- 快手：视频、图片和图文作品均使用统一长卡，视频文件独立发送。
- 微博：完整正文使用统一正文卡；无视频图文单图直接引用原图，多图、无封面正文卡和热门
  评论图合并转发；带视频微博的正文卡和视频仍按完成顺序发送。公开热门评论支持认证/VIP、
  原博标识、作者点赞和首条楼中楼；正文及评论中的微博表情均按图片渲染。
- 小红书：视频、图片和图文笔记均使用统一正文卡；图文单图直接引用原图，多图与无封面正文
  卡合并转发，视频文件独立发送；发布时间兼容秒和毫秒时间戳，空正文不会回填分享链接，
  平台内部的 `[话题]` 标记会清理为标准 `#话题#`。
- 米游社：文章正文、封面和正文配图合成一张图文一体长卡，成功后不再另发原图；视频文件
  独立发送，公开热门评论使用独立评论图合并转发，官方表情和用户自定义评论表情会按图片渲染。
- 小黑盒：帖子正文、封面和富文本配图按原顺序合成一张图文一体长卡，成功后不再重复发送
  帖子概要或富文本转发；视频独立发送，帖子评论使用独立评论图合并转发；官方表情会在长卡
  和评论图中显示，客户端本地缓存路径会被过滤；
...
运行环境还应提供 `ffmpeg`，用于音视频合并、格式转换和 H.264 发送兜底。统一正文卡依次展示
固定平台代表色纯色顶栏、真实互动数据、作者、正文、作品信息和
`Menkelo/astrbot_plugin_parser_x` 页脚；顶栏不再受作品图片颜色影响，也不再使用磨砂、颗粒、
模糊或 B/米/微等字母和单字平台标记。图文分离平台的作品图片不再充当正文卡封面；小黑盒
与米游社则在卡内按顺序展示正文配图。直播继续使用原生消息链；视频作品的正文卡与原生
视频文件并发准备，完成者先发送。
...
抖音与微博纯图文会把评论图并入原图/正文卡转发；其他作品使用独立评论转发，第一节点为平台
评论标题，后续每张评论图各占一个节点。合并转发失败时降级逐条发送，评论抓取或渲染失败不会
阻塞正文卡和媒体。B站、抖音、小红书与微博纯图文单图直接引用原图，多图与无封面正文卡进入
合并转发；小黑盒与米游社则将正文和配图合成一张长卡。视频作品的正文卡和视频文件按实际
完成顺序发送。
