...
- Bilibili：视频、分 P、多图/纯文字动态使用统一纵向长卡；单图动态引用原消息并在同一条
  消息中附带完整正文和原图，转发动态会补回原动态正文与图片；直播保持原生消息链。所有
...
- 抖音：视频、图文和图集；短链跳转；Cookie 与 yt-dlp 兜底；视频和图文均使用统一长卡，
  视频文件独立发送。配置 `douyin_ck` 后支持作者标识、表情、评论图片、贴纸和首条楼中楼，
  评论优先嵌入长卡。
- 快手：视频、图片和图文作品均使用统一长卡，视频文件独立发送。
- 微博：完整正文使用统一长卡；长卡单独引用原消息，原图和视频按各自批次随后发送。公开热门
  评论支持认证/VIP、原博标识、微博表情、作者点赞和首条楼中楼，并直接嵌入正文卡。
- 小红书：视频、图片和图文笔记均使用统一长卡，视频文件独立发送，发布时间兼容秒和毫秒
  时间戳。
- 米游社：文章使用统一长卡；长卡单独引用原消息，图片和视频按原生批次随后发送，公开热门
  评论直接嵌入长卡；官方表情和用户自定义评论表情会按图片渲染。
- 小黑盒：帖子与游戏使用统一长卡；长卡单独引用原消息，富文本仍按原图文顺序转发，视频
  独立发送，帖子评论直接嵌入长卡；官方表情会在长卡和富文本节点中显示，客户端本地缓存
  路径会被过滤；
...
其中视频兼容层会发送统一长卡和独立视频文件；网易云音乐仍直接发送音频文件。
...
动态取色顶栏、主视觉、真实互动数据、作者、正文、作品信息、热门评论和
`Menkelo/astrbot_plugin_parser_x` 页脚；顶栏优先提取作品图片主色，失败时回退平台品牌色，
不再显示 B/米/微等字母或单字平台标记。直播继续使用原生消息链；视频作品会先发送统一长卡，
再独立发送原生视频文件。
...
- `comments.timeout`：评论抓取、长卡合并和缓存生成的总超时。
...
启用统一正文卡时，插件会在限定时间内尝试抓取结构化热门评论并合并到同一张长卡。评论抓取、
片段生成或整卡合并失败时，正文会按无评论长卡或纯文本正常发送；不会再生成或发送旧式独立
评论长图。统一长卡始终作为引用原消息的独立消息，原图、富文本转发和视频不会与它合并；
合并转发中的每段文字和每张图片各占一个节点。
