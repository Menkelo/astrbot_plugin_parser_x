...
- Bilibili：视频、分 P、动态和直播。视频直接发送源视频，不渲染正文卡；图文动态单图使用
  “引用 + 正文卡 + 原图”，多图将正文卡和全部原图放入同一合并转发；直播保持原生消息链。
...
- 抖音：视频、图文和图集；短链跳转；Cookie 与 yt-dlp 兜底。视频与图片直接发送媒体，
  不渲染正文卡。配置 `douyin_ck` 后支持作品/评论
  表情、作者标识、评论图片、贴纸和首条楼中楼；评论区仅随视频作品发送。
- 快手：视频、图片和图文作品；媒体直接发送，不渲染正文卡，多图或
  图片与视频混合作品使用同一合并转发。
- 微博：纯文本渲染正文卡；带图片或视频时只发送媒体（单图引用原消息，多图合并转发），
  不再附加正文卡或文案。公开热门
